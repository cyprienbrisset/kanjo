...
// node est un nœud d'un petit arbre XML servant à générer de l'UBL avec des préfixes de
// namespace explicites (cac:, cbc:). encoding/xml ne permet pas de contrôler finement les
// préfixes ; cet arbre le fait proprement.
type node struct {
	name     string // nom qualifié, ex. "cbc:ID"
	attrs    []attr
...
// attribut est présent — utile pour les montants portant currencyID).
